...
class AttributesNetwork(nn.Module):
    ...
    def __init__(self, num_attributes: int):
        ...
        self._conv1 = create_conv_layer(in_chan=3, out_chan=16)
        self._conv2 = create_conv_layer(in_chan=16, out_chan=32)
        self._conv3 = create_conv_layer(in_chan=32, out_chan=64)
        self._conv4 = create_conv_layer(in_chan=64, out_chan=64)
        self._conv5 = create_conv_layer(in_chan=64, out_chan=32)

        self._lin1 = nn.Linear(in_features=32*32*32, out_features=1024)
        self._lin2 = nn.Linear(in_features=1024, out_features=64)
