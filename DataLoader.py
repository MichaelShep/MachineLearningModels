...
class ImageDataset:
  ...
  def __getitem__(self, idx):
    ...
    #For now, only return the hair map as output
    output_elements = self.ids[idx][1]

    for i in range(len(output_elements)):
      if 'hair' in self._get_file_name_string(output_elements[i]):
        output_image = self._tensor_transform(Image.open(output_elements[i]))
      else:
        output_image = None
