...
class AttributesNetwork(nn.Module):
    ...
    def __init__(self, num_attributes: int, device: str):
        ...
        self._res_layers = nn.ModuleList([
            create_conv_layer(in_chan=3, out_chan=8), create_conv_layer(in_chan=8, out_chan=8),
            create_conv_layer(in_chan=8, out_chan=16), create_conv_layer(in_chan=16, out_chan=16),
            create_conv_layer(in_chan=16, out_chan=32), create_conv_layer(in_chan=32, out_chan=32),
            create_conv_layer(in_chan=32, out_chan=64), create_conv_layer(in_chan=64, out_chan=64),
            create_conv_layer(in_chan=64, out_chan=128), create_conv_layer(in_chan=128, out_chan=128),
            create_conv_layer(in_chan=128, out_chan=256), create_conv_layer(in_chan=256, out_chan=256),
        ])
    ...
    '''
    def _perform_linear_layer(self, x: torch.Tensor, idx: int, dropout: bool) -> torch.Tensor:
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        for i in range(len(self._lin_layers) - 1):
            if i == 0:
                x = self._perform_linear_layer(x, i, False)
            else:
                x = self._perform_linear_layer(x, i, False)
